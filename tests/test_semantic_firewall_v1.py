...
class SemanticFirewallV1Tests(unittest.TestCase):
    ...
    def snapshot(self, *, state="KNOWN", source_type="LIVE_GITHUB", observed_at=FRESH, value="AUTHORIZED"):
    ...
    def trusted_receipt(self):
        ...
        receipt = sf.make_receipt_candidate(
            ...
            evaluated_at=NOW,
            expires_at="2026-09-13T20:10:00Z",
        )
    ...
    def test_exact_bound_evidence_passes(self):
        contract = self.contract()
        snapshot = self.snapshot()
    ...
    def test_unknown_required_input_is_not_run(self):
        contract = self.contract()
        snapshot = self.snapshot(state="UNKNOWN")
        result = sf.evaluate_control(contract, self.request(contract, snapshot), snapshot, evaluated_at=NOW)
    ...
    def test_stale_required_input_is_not_run(self):
        contract = self.contract()
        snapshot = self.snapshot(observed_at=STALE)
        result = sf.evaluate_control(contract, self.request(contract, snapshot), snapshot, evaluated_at=NOW)
        self.assertEqual("NOT_RUN", result["decision"])
    ...
    def test_future_observation_fails(self):
        contract = self.contract()
        snapshot = self.snapshot(observed_at=FUTURE)
        result = sf.evaluate_control(contract, self.request(contract, snapshot), snapshot, evaluated_at=NOW)
        self.assertEqual("FAIL", result["decision"])
        self.assertIn("INPUT_OBSERVED_IN_FUTURE", result["reason_codes"])
    ...
    def test_contract_hash_mismatch_fails(self):
        ...
        result = sf.evaluate_control(contract, request, snapshot, evaluated_at=NOW)
        self.assertEqual("FAIL", result["decision"])
        self.assertIn("CONTRACT_HASH_MISMATCH", result["reason_codes"])
    ...
    def test_source_candidate_receipt_has_zero_effect_eligibility(self):
        ...
        effect = self.effect_request(receipt)
        eligibility = sf.evaluate_effect_eligibility(receipt, effect, evaluated_at=NOW)
    ...
    def test_operation_digest_mismatch_fails_effect_eligibility(self):
        receipt = self.trusted_receipt()
        effect = self.effect_request(receipt)
        effect["operation_sha256"] = "b" * 64
        result = sf.evaluate_effect_eligibility(receipt, effect, evaluated_at=NOW)
        self.assertEqual("FAIL", result["decision"])
        self.assertIn("OPERATION_HASH_MISMATCH", result["reason_codes"])

    def test_expired_trusted_receipt_is_not_run(self):
        receipt = self.trusted_receipt()
        receipt["evaluated_at"] = "2026-09-13T19:50:00Z"
        receipt["expires_at"] = "2026-09-13T19:59:59Z"
        receipt["receipt_sha256"] = sf.recompute_receipt_sha(receipt)
        effect = self.effect_request(receipt)
        result = sf.evaluate_effect_eligibility(receipt, effect, evaluated_at=NOW)
        self.assertEqual("NOT_RUN", result["decision"])
    ...
    def test_post_effect_requires_authoritative_verified_effect_and_postconditions(self):
        ...
        verification = {
            ...
            "operation_sha256": effect["operation_sha256"],
            "effect_state": "EFFECT_VERIFIED",
            "required_postconditions_state": "PASS",
            "readback_source_class": "AUTHORITATIVE_READBACK",
            "readback_ref": "github:readback:1",
            "observed_at": NOW,
        }
    ...
    def test_unknown_effect_fails_closed_as_not_run(self):
        ...
        verification = {
            ...
            "required_postconditions_state": "UNKNOWN",
            "readback_source_class": "AUTHORITATIVE_READBACK",
            "readback_ref": "github:readback:unknown",
            "observed_at": NOW,
        }
        result = sf.evaluate_post_effect(effect, verification, evaluated_at=NOW)
        self.assertEqual("NOT_RUN", result["decision"])
        self.assertIn("EFFECT_STATE_INCOMPLETE", result["reason_codes"])
        self.assertIn("POSTCONDITIONS_INCOMPLETE", result["reason_codes"])
    ...
    def test_duplicate_required_input_name_fails(self):
        ...
        result = sf.evaluate_control(contract, self.request(contract, snapshot), snapshot, evaluated_at=NOW)
        self.assertEqual("FAIL", result["decision"])
        self.assertIn("DUPLICATE_OR_INVALID_REQUIRED_INPUT_NAME", result["reason_codes"])

    def test_future_request_time_fails(self):
        contract = self.contract()
        snapshot = self.snapshot()
        request = self.request(contract, snapshot)
        request["requested_at"] = FUTURE
        result = sf.evaluate_control(contract, request, snapshot, evaluated_at=NOW)
        self.assertEqual("FAIL", result["decision"])
        self.assertIn("REQUESTED_AT_IN_FUTURE", result["reason_codes"])

    def test_missing_write_ahead_preparation_fails_effect_eligibility(self):
        receipt = self.trusted_receipt()
        effect = self.effect_request(receipt)
        effect["prepared_attempt_ref"] = ""
        result = sf.evaluate_effect_eligibility(receipt, effect, evaluated_at=NOW)
        self.assertEqual("FAIL", result["decision"])
        self.assertIn("WRITE_AHEAD_PREPARATION_MISSING", result["reason_codes"])

    def test_future_readback_fails_post_effect(self):
        receipt = self.trusted_receipt()
        effect = self.effect_request(receipt)
        verification = {
            "schema_version": 1,
            ...
            "required_postconditions_state": "PASS",
            "readback_source_class": "AUTHORITATIVE_READBACK",
            "readback_ref": "github:readback:future",
            "observed_at": FUTURE,
        }
        result = sf.evaluate_post_effect(effect, verification, evaluated_at=NOW)
        self.assertEqual("FAIL", result["decision"])
